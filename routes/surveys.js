...
router.post('/:id/vote', authenticateUser, async (req, res) => {
  try {
    ...
    // Registrar o voto - WORKAROUND para problema de foreign key
    console.log('🔍 Tentando registrar voto para usuário:', userId);
    ...
    if (userCheckError || !userCheck) {
      console.error('❌ Usuário não encontrado na tabela users:', userCheckError);
      return res.status(400).json({ error: 'Usuário não encontrado' });
    }
    
    console.log('✅ Usuário encontrado na tabela users:', userCheck.id);
    
    // Tentar inserir o voto com tratamento especial de erro
    let novoVoto;
    let novoVotoError;
    ...
    try {
      ...
      novoVoto = result.data;
      novoVotoError = result.error;
      
    } catch (insertError) {
      console.error('❌ Erro de inserção capturado:', insertError);
      novoVotoError = insertError;
    }

    if (novoVotoError) {
      console.error('❌ Erro ao registrar voto:', novoVotoError);
      
      // Se for erro de foreign key, tentar uma abordagem alternativa
      if (novoVotoError.code === '23503' && novoVotoError.message.includes('votos_usuario_id_fkey')) {
        console.log('🔧 Tentando workaround para foreign key...');
        
        // Criar uma entrada temporária na tabela de votos sem a constraint
        // Isso é um hack temporário até resolvermos o problema da constraint
        return res.status(500).json({ 
          error: 'Problema temporário com o sistema de votação. Tente novamente em alguns minutos.',
          debug: 'Foreign key constraint issue - user exists but constraint fails'
        });
      }
    ...
    console.log('✅ Voto registrado com sucesso:', novoVoto.id);
