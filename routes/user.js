...
router.get('/usage-stats', authenticateUser, async (req, res) => {
  try {
    ...
    for (const feature of features) {
      try {
        const limitCheck = await checkUserLimits(userId, feature);
        
        usageStats[feature] = {
          used: limitCheck.used || 0,
          limit: limitCheck.limit || 0,
          remaining: limitCheck.remaining || 0,
          canUse: limitCheck.canUse || false
        };
      } catch (error) {
...
router.get('/usage-history', authenticateUser, async (req, res) => {
  try {
    ...
    // Buscar histórico de conversas (IA criativa e agentes políticos)
    ...
    if (conversationError) {
      console.error('Erro ao buscar histórico de conversas:', conversationError);
    }
    ...
    // Contar conversas por dia e tipo
