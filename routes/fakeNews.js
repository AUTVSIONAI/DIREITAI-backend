...
const { analyzeFakeNews, checkUserLimits, checkUserCredits, consumeUserCredit } = require('../services/aiService');
...
router.post('/analyze', authenticateUser, async (req, res) => {
  try {
    ...
    const limits = await checkUserLimits(userId, req.user.plano, 'fake_news');
    
    // Se não pode usar pelo plano, verificar se tem créditos avulsos
    let usingCredits = false;
    if (!limits.canUse && !limits.unlimited) {
      const credits = await checkUserCredits(userId, 'fake_news_check');
      
      if (!credits.hasCredits) {
        return res.status(429).json({
          error: 'Limite diário excedido',
          message: `Você atingiu o limite de ${limits.limit} análises por dia. Faça upgrade do seu plano ou compre créditos avulsos por R$ 1,50 cada.`,
          limits: limits,
          credits: credits,
          upgrade_options: {
            plans: ['engajado', 'lider', 'supremo'],
            credit_price: 1.50
          }
        });
      }
    ...
    // Preparar conteúdo para salvar no banco
    let contentToSave;
    if (type === 'imagem' && content.startsWith('data:image/')) {
      // Para imagens, salvar apenas uma descrição ao invés da URL base64 gigantesca
      contentToSave = 'Imagem enviada para análise';
    } else {
      // Para texto e links, limitar o tamanho
      contentToSave = content.substring(0, 1000);
    }
    ...
    // Se usou créditos, consumir um crédito
    if (usingCredits) {
      await consumeUserCredit(userId, 'fake_news_check');
    }
...
router.post('/:id/feedback', authenticateUser, async (req, res) => {
  try {
    ...
    console.log('🔍 FEEDBACK REQUEST:', {
      ...
      userId,
      tipo_feedback,
      comentario: comentario ? 'presente' : 'ausente'
    });

    if (!['concordo', 'discordo', 'denuncia'].includes(tipo_feedback)) {
      console.log('❌ Tipo de feedback inválido:', tipo_feedback);
    ...
    console.log('🔍 Verificando se check existe:', id);
    ...
    if (checkError) {
      console.log('❌ Erro ao buscar check:', checkError);
    ...
    console.log('✅ Check encontrado:', check.id);

    // Inserir feedback
    console.log('🔍 Inserindo feedback na tabela fake_news_feedback...');
    ...
    if (feedbackError) {
      console.log('❌ Erro ao salvar feedback:', feedbackError);
      return res.status(500).json({ 
    ...
    console.log('✅ Feedback salvo:', feedback.id);

    // Atualizar contadores na tabela principal
    console.log('🔍 Atualizando contadores...');
    try {
      if (tipo_feedback === 'concordo') {
        console.log('🔍 Incrementando feedback positivo...');
        const { error: rpcError } = await supabase.rpc('increment_feedback_positivo', { check_id: id });
        if (rpcError) {
          console.log('❌ Erro na RPC increment_feedback_positivo:', rpcError);
        } else {
          console.log('✅ Feedback positivo incrementado');
        }
      } else if (tipo_feedback === 'discordo') {
        console.log('🔍 Incrementando feedback negativo...');
        const { error: rpcError } = await supabase.rpc('increment_feedback_negativo', { check_id: id });
        if (rpcError) {
          console.log('❌ Erro na RPC increment_feedback_negativo:', rpcError);
        } else {
          console.log('✅ Feedback negativo incrementado');
        }
      } else if (tipo_feedback === 'denuncia') {
        console.log('🔍 Incrementando denúncias...');
        const { error: rpcError } = await supabase.rpc('increment_denuncias', { check_id: id });
        if (rpcError) {
          console.log('❌ Erro na RPC increment_denuncias:', rpcError);
        } else {
          console.log('✅ Denúncias incrementadas');
        }
      }
    } catch (rpcError) {
      console.log('❌ Erro ao executar RPC:', rpcError);
      // Não retornar erro aqui, pois o feedback já foi salvo
    }
    ...
    console.error('❌ ERRO GERAL no endpoint de feedback:', error);
    res.status(500).json({ 
      error: 'Erro interno do servidor',
      details: error.message
    });
